...
export const findByIdAsync = (id) => async (dispatch) => {
    try {
        ...
        console.log(token)
        const response = await axios.get(buildEndpointURL(ENDPOINTS.PRODUCT.DETAIL + id),
            {
                headers: { Authorization: `Bearer ${token}` }
            });
        console.log(response)
...
export const updateAsync = () => async (dispatch) => {
    try {
        ...
        const token = user.access_token;
        console.log(token)
        const response = await axios.put(buildEndpointURL(ENDPOINTS.PRODUCT.UPDATE),
            {
                headers: { Authorization: `Bearer ${token}` }
            });
        console.log(response)
...
export const destroyAsync = () => async (dispatch) => {
    try{
        ...
        const token =user.access_token
        console.log(token)
        const response = await axios.delete(buildEndpointURL(ENDPOINTS.PRODUCT.DELETE),
        {
            headers :{ Authorization:`Bearer ${token}` } 
        },
        )
